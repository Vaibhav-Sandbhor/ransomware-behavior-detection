...
import {
  ...
  startHoneypotSimulation,
  startHoneypotMonitor,
  stopHoneypotScan,
  getHoneypotScanStatus,
} from "../../services/api";
...
export default function HoneypotDetails({ moduleState, updateModuleState }) {
  ...
  // Honeypot dual-mode scanning
  const [scanMode, setScanMode] = useState(null);  // null | "simulation" | "monitor"
  ...
  useEffect(() => {
    ...
    if (moduleState.events?.length) setEvents(moduleState.events);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const total = Number(logData.summary?.total ?? 0);
      const risky = Number(logData.summary?.critical ?? 0) + Number(logData.summary?.warning ?? 0);

      updateModuleState({
        count: total,
        status: risky > 0 ? "MALICIOUS" : "SAFE",
  ...
  const handleStartSimulation = async () => {
    try {
      ...
      await startHoneypotSimulation();

      // Start status polling every 2 seconds
      const interval = setInterval(async () => {
        try {
          const status = await getHoneypotScanStatus();
          if (!status.active) {
            clearInterval(interval);
            setStatusPollInterval(null);
            setIsScanning(false);
            // Load results when scan completes
            await loadData();
          }
        } catch (e) {
      ...
      setError(`Failed to start honeypot simulation: ${e.message}`);
      setIsScanning(false);
      setScanMode(null);
    }
  };
  ...
  const handleStartMonitor = async () => {
    try {
      ...
      setError(`Failed to start honeypot monitor: ${e.message}`);
      setIsScanning(false);
      setScanMode(null);
    }
  };
  ...
  const handleStopScan = async () => {
    try {
      ...
      setError(`Failed to stop honeypot scan: ${e.message}`);
    }
  };
    ...
    <div className="report-page" style={{ color: "#e0e0e0" }}>
      ...
      <div style={{ marginBottom: 24, display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        <button
          ...
          disabled={isScanning}
          style={{
            background: scanMode === "simulation" ? "#7c83fd" : "transparent",
            color: scanMode === "simulation" ? "#fff" : "#aaa",
            border: `1px solid ${scanMode === "simulation" ? "#7c83fd" : "#555"}`,
            cursor: isScanning && scanMode !== "simulation" ? "not-allowed" : "pointer",
          }}
        >
          {isScanning && scanMode === "simulation" ? "Scanning..." : "Honeypot Simulation"}
        </button>
        ...
        <button
          ...
          disabled={isScanning}
          style={{
            background: scanMode === "monitor" ? "#26c6da" : "transparent",
            color: scanMode === "monitor" ? "#fff" : "#aaa",
            border: `1px solid ${scanMode === "monitor" ? "#26c6da" : "#555"}`,
            cursor: isScanning && scanMode !== "monitor" ? "not-allowed" : "pointer",
          }}
        >
          {isScanning && scanMode === "monitor" ? "Monitoring..." : "Honeypot System Monitor"}
        </button>
        ...
        <button className="scan-btn" onClick={loadData} disabled={loading || isScanning}>
